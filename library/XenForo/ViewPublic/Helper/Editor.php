	...
	public static function supportsRichTextEditor()
	{
		...
		if (!empty($_SERVER['HTTP_USER_AGENT']))
		{
			...
			)
			{
				...
				preg_match('#windows phone (\d+).\d+;.+Edge/(\d+).\d+#i', $_SERVER['HTTP_USER_AGENT'], $match)
			)
			{
				list ($match, $osMajor, $edgeMajor) = $match;
				if ($osMajor == 10 && $edgeMajor < 14)
				{
					// Windows Phone 10 with Edge 13 has various editor related issues
					$showWysiwyg = false;
				}
			}
